# Visual Studio Code

In these materials we will be using Visual Studio Code to write HTML and JavaScript files. VS Code is source software that is available for Windows, macOS and Linux operating systems.

Install Visual Studio Code.

1. Go to the <a href="https://code.visualstudio.com/" target="_blank">Visual Studio Code</a> website.
2. Download and install VS Code for your operating system.
3. Open VS Code once installation is complete.

Install the Live Server extension.

Live Server allows you to run web pages locally and automatically reload them when files change.

1. Open VS Code
2. Click the Extensions icon in the left sidebar (or press <kbd>ctrl</kbd> + <kbd>shift</kbd> + <kbd>X</kbd> on Windows or Linux, or <kbd>&#8984;</kbd> + <kbd>shift</kbd> + <kbd>X</kbd> on macOS)
3. Search for Live Server
4. Install Live Server by Ritwick Dey

Once installed, VS Code may prompt you to reload.
